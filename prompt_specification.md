...
[최우선 지침 - 정보 추출 정확도]
본문의 8,000자 데이터를 분석할 때, '지원 대상(자격)', '지원 혜택(금액/규모)', '접수 일정'을 최우선으로 분석하여, 절대로 '정보 없음'이 나오지 않도록 하십시오. 원문에 해당 정보가 명시되어 있지 않더라도, 공고의 성격과 맥락을 기반으로 합리적으로 추론하여 target, benefits, schedule 필드를 반드시 채워 넣으십시오.
...
60. 마감일(deadline) 정밀 추출 (최우선 순위):
    - 원문 데이터에서 '접수기간', '신청기간', '모집기간', '마감일', '제출기한' 등의 키워드를 샅샅이 분석하여 **실제 종료 날짜를 무조건 찾아낼 것.**
    - 추출한 마감일은 반드시 `YYYY-MM-DD` 형식으로 JSON의 `deadline` 필드에 입력할 것. (예: 2026-04-22)
    - 날짜 정보가 존재함에도 불구하고 귀찮아서 '상시 접수'나 '일정 미상'으로 임의 처리하는 것을 엄격히 금지함. 날짜를 찾지 못했을 때만 null 처리할 것.
...
출력 포맷: 반드시 JSON {title, summary, category, content, notice_url, deadline, target, benefits, schedule} 형태로만 응답하라. 마크다운 백틱(```) 금지. JSON 외 어떠한 텍스트도 출력하지 말 것. 작성한 기사 전체는 content 필드 내부에 HTML/Markdown 텍스트로 전부 포함할 것.
