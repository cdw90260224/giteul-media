...
61. 첨부파일 보존 (절대 지침):
    - 딥 크롤링 데이터 내에 '[첨부파일 메타데이터]' 항목이 존재할 경우, 기사 본문 가장 하단에 '### 📎 첨부파일' 섹션을 생성하고 해당 메타데이터(파일명 및 링크)를 절대 누락 없이 100% 포함시킬 것.
    - 원본 데이터에 첨부파일 정보가 없을 경우에만 해당 섹션을 생략할 것. (임의로 생성하거나 거짓말하지 말 것)
