...
const getEnv = (key: string) => env.match(new RegExp(`${key}=(.*)`))?.[1]?.trim() || process.env[key] || '';
...
async function main() {
    ...
    for (let i = 0; i < posts.length; i++) {
        ...
        // Check if already contains new sections to avoid duplicate work if possible
        if (post.content.includes('최대 지원 금액') && post.content.includes('우대 사항')) {
        ...
        if (!deepContext) {
            console.log(`  -> No text extracted from attachments. Skipping regeneration.`);
            continue;
        }
