...
// Use the exact fixed logic from newsletter-engine.ts
      ...
      <div style="margin-bottom: 40px;">
        ...
        ${section.news.map((item) => `
          <div style="margin-bottom: 25px; padding-bottom: 20px; border-bottom: 1px solid #f1f5f9;">
             ...
             <a href="https://giteul.media/article/${item.id}?utm_source=newsletter" style="color: #0f172a; font-size: 12px; text-decoration: none; font-weight: 800; border-bottom: 1.5px solid #e2e8f0; padding-bottom: 2px;">분석 리포트 보기 →</a>
          </div>
        ...
        <div style="display: block;">
          ${section.support.map(item => `
            <div style="background: #ffffff; padding: 24px; border-radius: 20px; border: 1px solid #f1f5f9; margin-bottom: 16px; box-shadow: 0 4px 6px -1px rgba(0,0,0,0.03);">
              <h4 style="margin: 0 0 15px 0; color: #0f172a; font-size: 15px; font-weight: 900; line-height: 1.5;">${item.title}</h4>
              <div style="display: table; width: 100%;">
                ...
                <div style="display: table-cell; vertical-align: middle; text-align: right;">
                  <a href="https://giteul.media/article/${item.id}?utm_source=newsletter" style="font-size: 13px; font-weight: 900; color: #64748b; text-decoration: none;">상세보기</a>
                </div>
              </div>
            </div>
...
async function finalTest() {
    ...
    if (sections.length > 0) {
        ...
        await transporter.sendMail({
            ...
            to: sub.email,
            subject: `[최종 확인] 기틀 통합 비즈니스 리포트`,
            html: html
        });
        console.log('Final Test Sent Successfully.');
    }
}
finalTest();
